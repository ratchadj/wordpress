...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( $is_comment == 1 ) {
		$post_id_class = esc_attr( ' wn-like-comment-button-' . $post_id );
		$comment_class = esc_attr( ' wn-like-comment' );
		$like_count = get_comment_meta( $post_id, "_comment_like_count", true );
		$like_count = ( isset( $like_count ) && is_numeric( $like_count ) ) ? $like_count : 0;
	} else {
		$post_id_class = esc_attr( ' hcolorf wn-like-button-' . $post_id );
	...
	// Loader
	$loader = '<span id="wn-like-loader"></span>';
	// Liked/Unliked Variables
	if ( already_liked( $post_id, $is_comment ) ) {
		$class = esc_attr( ' liked' );
		$title = __( 'Unlike', 'deep' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'Like', 'deep' );
	...
	$output = '<span class="wn-like-wrapper"><a href="' . admin_url( 'admin-ajax.php?action=process_simple_like' . '&post_id=' . $post_id . '&nonce=' . $nonce . '&is_comment=' . $is_comment . '&disabled=true' ) . '" class="wn-like-button' . $post_id_class . $class . $comment_class . '" data-nonce="' . $nonce . '" data-post-id="' . $post_id . '" data-iscomment="' . $is_comment . '" title="' . $title . '">' . $count . $icon . '</a>' . $loader . '</span>';
	return $output;
...
function get_unliked_icon() {
	/* If already using Font Awesome with your theme, replace svg with: <i class="wn-fa wn-fa-heart-o"></i> */
...
function get_like_count( $like_count ) {
	...
	$count = '<div class="wn-like-count"><span class="wn-like-counter">' . $number . '</span></div>';
	return $count;
} // get_like_count()
...
function show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'deep' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php _e( 'You do not like anything yet.', 'deep' ); ?></p>
			<?php
			endif;
			wp_reset_postdata();
			?>
